...
class DamageEventExtractor:
    ...
    def total_damage_stat(self, damage_df, total_round_num):
        ...
        combined_df = combined_df.applymap(format_float)  # 将所有浮点数进行格式化
        return combined_df
    ...
    def without_general_damage_stat(self, df_damage_cleaned, total_round_num):
        ...
        df_without_location_general[['causerID', 'isHeadShot']] = df_without_location_general['damageEvent'].apply(
            lambda x: pd.Series({
                'causerID': x.get('causerId', {}).get('value', None),
...
if __name__ == '__main__':
    ...
    print(damage_event_df)
    #数据合并
    # player_data = pd.merge(team_info_df, damage_event_df, on='PlayerID', how='inner')
    # print(player_data)
